  ...
  // 등기접수임박
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('V')->setWidth(50);
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("E".$i, "잔금일")
                 ->setCellValue("F".$i, "은행1")
                 ->setCellValue("G".$i, "지점1")
                 ->setCellValue("H".$i, "특이사항")
                 ->setCellValue("I".$i, "설정갯수")
                 ->setCellValue("J".$i, "미비서류")
                 ->setCellValue("K".$i, "서류유효기간")
                 ->setCellValue("L".$i, "취득세납부일")
                 ->setCellValue("M".$i, "본인납부")
                 ->setCellValue("N".$i, "수납결과")
                 ->setCellValue("O".$i, "인지전달일")
                 ->setCellValue("P".$i, "(예상)등기접수일")
                 ->setCellValue("Q".$i, "위임장요청일")
                 ->setCellValue("R".$i, "위임장수령일")
                 ->setCellValue("S".$i, "건축대장요청일")
                 ->setCellValue("T".$i, "등기접수일")
                 ->setCellValue("U".$i, "접수비고")
                 ->setCellValue("V".$i, "정산비고")
                 ;
	$i++;
    ...
    if($row[al1_hope_yn]=="y"||$row[al1_imp_yn]=="y"){
	    if(($row[total_sum]-$row[al1]-$row[ai1])!=0){
	       $c_temp = f_money($row[total_sum]-$row[al1]-$row[ai1]) ;
	    }else{
	       $c_temp = "ok";
	    }
    } else{	
      if(($row[total_sum]-$row[ai1])!=0){
	       $c_temp = f_money($row[total_sum]-$row[ai1]) ;
	    }else{
	       $c_temp = "ok";
	    }
    }
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("E".$i, $row[balance_date])
                 ->setCellValue("F".$i, f_bank_name($row[d1]))
                 ->setCellValue("G".$i, f_jijum_name($row[e1]))
                 ->setCellValue("H".$i, $row[point_dm])
								 ->setCellValue("I".$i, $row[f1])
								 ->setCellValue("J".$i, $a_temp)
								 ->setCellValue("K".$i, $b_temp)
								 ->setCellValue("L".$i, $row[r1])
								 ->setCellValue("M".$i, $row[al1_imp_yn])
								 ->setCellValue("N".$i, $c_temp)
								 ->setCellValue("O".$i, $row[am1_relay_date])
								 ->setCellValue("P".$i, $row[pred_g1])
								 ->setCellValue("Q".$i, $row[man_req_date])
								 ->setCellValue("R".$i, $row[man_rec_date])
								 ->setCellValue("S".$i, $row[bdoc_req_date])
								 ->setCellValue("T".$i, $row[g1])
								 ->setCellValue("U".$i, $row[rec_memo])
								 ->setCellValue("V".$i, $row[ae1])
                 ;
	$i++;
	}
	 ...
	 $objPHPExcel->setActiveSheetIndex(0); 
	 $filename = iconv("UTF-8", "EUC-KR", "등기관리팀리스트_엑셀");
