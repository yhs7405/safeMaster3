<?	include ("../include/dbconn.php");
	...
	if($target_date=="") $target_date="sg_b_date";
	...
	if($target_date!=""){
		if(($s_date!="")&&($e_date!="")){
			...
			if(($target_date=="sjp_s_date")||($target_date=="sjp_j_date")||($target_date=="sjj_w_date")||($target_date=="sjp_b_date")){  //설정일때
				...
				if($target_date!="") {$imsi = " and b.{$target_date} between ";}
	...
	$sql = "select j.*,b.* from  tbl_junib j cross join tbl_suljung b on j.a1=b.a1   $wherequery order by j.idx asc ";
	//echo $sql;
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(20);
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("M".$i, "세금계산서발행일")
                 ->setCellValue("N".$i, "국토교통부");
	$i++;
	...
	while($row = $stmt->fetch()){
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("M".$i, f_date($row[sg_b_date]))
                 ->setCellValue("N".$i, f_gukto($row[gukto]));
	$i++;
	}
	 ...
	 $objPHPExcel->setActiveSheetIndex(0); 
	 $filename = iconv("UTF-8", "EUC-KR", "erp_세금계산서발행일");
