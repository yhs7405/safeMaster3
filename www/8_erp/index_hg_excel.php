...
//	}
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(20);
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("G".$i, "보수액")
                 ->setCellValue("H".$i, "공과금")
                 ->setCellValue("I".$i, "총비용")
                 ->setCellValue("J".$i, "입금")
                 ->setCellValue("K".$i, "정산(환불금)")
                 ->setCellValue("L".$i, "이전영수증출력일")
                 ->setCellValue("M".$i, "현금영수증발행일");
	$i++;
	...
	while($row = $stmt->fetch()){
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("G".$i, $bosu)
                 ->setCellValue("H".$i, $gongga)
                 ->setCellValue("I".$i, $bosu+$gongga)
                 ->setCellValue("J".$i, $row[ai1])
                 ->setCellValue("K".$i, $suljung_bosu+$suljung_gongga)
                 ->setCellValue("L".$i, $row[refund_fee])
                 ->setCellValue("M".$i, f_date($row[ijy_c_date]))
                 ->setCellValue("N".$i, f_date($row[hy_b_date]));
	$i++;
	}
	 ...
	 $objPHPExcel->setActiveSheetIndex(0); 
	 $filename = iconv("UTF-8", "EUC-KR", "erp_현금영수증발행내역");
