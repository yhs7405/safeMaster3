<?	include ("../../include/dbconn.php");
	...
	for($i=1;$i<=$list_num;$i++){		
		if($_REQUEST["idx_".$i]!=""){
			...
			$sql = "update tbl_junib set spj_update='{$datex}' where idx=".$_REQUEST["idx_".$i]." ";
				//echo $sql;
	...
	for($i=1;$i<=$list_num;$i++){
		for($x=1;$x<5;$x++){
			if(trim($_REQUEST["x_sjp_j_date".$x."_".$i])=="y"){
				$sql = "update  tbl_suljung set sjp_j_date='".$_REQUEST["sjp_j_date".$x."_".$i]."',sjp_j_id='".$_SESSION["admin_id"]."' where suljung_no={$x} and a1='".$_REQUEST["a1_".$i]."'";
				//echo $sql."<br>";
				db_query($sql);
			}
		}
