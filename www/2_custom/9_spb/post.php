<?	include ("../../include/dbconn.php");
	...
	for($i=1;$i<=$list_num;$i++){
		...
		if($_REQUEST["idx_".$i]!=""){
			...
			$sql = "update tbl_junib set spb_update='{$datex}' where idx=".$_REQUEST["idx_".$i]." ";
			//echo $sql;
	...
	for($i=1;$i<=$list_num;$i++){
		for($x=1;$x<5;$x++){
			if(trim($_REQUEST["x_sjp_b_date".$x."_".$i])=="y"){
				$sql = "update  tbl_suljung set sjp_b_date='".$_REQUEST["sjp_b_date".$x."_".$i]."',sjp_b_id='".$_SESSION["admin_id"]."' where suljung_no={$x} and a1='".$_REQUEST["a1_".$i]."'";
				//echo $sql."<br>";
				db_query($sql);
			}
		}
