...
//	ini_set("display_errors", 1);
	...
	for($i=1;$i<=$list_num;$i++){
		if($_REQUEST["idx_".$i]!=""){
			if(trim($_REQUEST["ijp_s_date1_".$i])=="y"){
				$sql = "update  tbl_junib set ijp_s_date='".$_REQUEST["ijp_s_date_".$i]."',ijp_s_id='".$_SESSION["admin_id"]."' where idx=".$_REQUEST["idx_".$i];
				//echo $sql."<br>";
				db_query($sql);
			}
				...
				$sql = "update tbl_junib set ijp_s_update='{$datex}' where idx=".$_REQUEST["idx_".$i]." ";
				//echo $sql;
