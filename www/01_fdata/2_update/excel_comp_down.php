<?	include ("../../include/dbconn.php");
  ...
  // as1(cs비고) , au1(은행비고)
	...
	if(	$bank_code!=""&&$jijum_code	!=""){
     $objPHPExcel->setActiveSheetIndex(0)
                 ->setCellValue("A".$i, "■ ".f_hyunjang_name($h_idx)."(".f_money0($rows_total).")건"." [".f_bank_name($bank_code)."-".f_jijum_name($jijum_code)."]");
	} else {
     $objPHPExcel->setActiveSheetIndex(0)
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth(40);
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("G".$i, "등기원인일")
                 ->setCellValue("H".$i, "채권최고액1")
                 ->setCellValue("I".$i, "채권최고액2")
                 ->setCellValue("J".$i, "채권최고액3")
                 ->setCellValue("K".$i, "채권최고액4")
                 ->setCellValue("L".$i, "은행비고")
                 ;
	$i++;
	...
	while($row = $stmt->fetch()){
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("H".$i, $row[av1])
                 ->setCellValue("I".$i, $row[ax1])
                 ->setCellValue("J".$i, $row[ay1])
                 ->setCellValue("K".$i, $row[az1])
                 ->setCellValue("L".$i, $row[au1])
                 ;
	$i++;
	}
	 ...
	 $objPHPExcel->setActiveSheetIndex(0); 
	 $filename = iconv("UTF-8", "EUC-KR", "채권최고액확인요청_엑셀");
