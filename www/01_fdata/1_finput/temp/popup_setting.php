          ...
          <div class="widget-content nopadding">
            ...
            <div class="widget-content nopadding">
			        ...
			        <ul class="nav nav-tabs">
			            ...
			            <li>
			                <a href="popup_setting_b.php">&nbsp;&nbsp;&nbsp;현&nbsp;&nbsp;장&nbsp;&nbsp;&nbsp; </a>
			            </li>
	        		</ul>
        	</div>
			  ...
			  <thead>
				<tr>
				  <th style="text-align:center;margin-top:20px;" valign="top">
					...
					은행&nbsp;<select name="bank_code" id="bank_code" >
...
//	echo $sql;
	...
	if($rows > 0){
		...
		while($row = $stmt->fetch()){?>
                ...
                <tr class="odd gradeX">
                  <td style="text-align:center;"><a href="javascript:f_idx('<?=$row[bank_code]?>','<?=$row[gubun_code]?>');"><?=($page-1)*$view_num+$T?></a></td>
                  <td style="text-align:center;"><a href="javascript:f_idx('<?=$row[bank_code]?>','<?=$row[gubun_code]?>');"><?=f_bank_code_name($row[bank_code])?></a></td>
                  <td style="text-align:center;"><a href="javascript:f_idx('<?=$row[bank_code]?>','<?=$row[gubun_code]?>');"><?=f_gubun_name($row[gubun_code])?></a></td>
                </tr>
...
function f_idx(bank_code,gubun_code){//은행비율정보 가져오기
	if(confirm("적용하시겠습니까?")){
		...
		if(bank_code!=""){
			...
			$.ajax({
				...
				url:'./ajax_bank.php',
				dataType:"json",
				data:{bank_code:bank_code,gubun_code:gubun_code},
				timeout : 30000,
