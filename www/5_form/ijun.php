<?	include ("../include/dbconn.php");
	...
	if($kikan2_null_ch=="Y"){
		if(($target_date2=="sjp_s_date")||($target_date2=="sjp_j_date")||($target_date2=="sjj_w_date")||($target_date2=="sjp_b_date")||($target_date2=="sg_b_date")){ //설정일때
  ...
  $sqlaa=$wherequery;
	function f_total($where){
		$sql = "select sum(aq1+ar1+as1+at1) as bosu,sum(aj1+ak1+al1+am1+an1+ao1+ap1) as gongga,";
		$sql.= " sum(aq1+ar1+as1+at1+aj1+ak1+al1+am1+an1+ao1+ap1) as total_fee,";
		$sql.= " sum(ai1) as ibgum,sum(refund_fee) as refund ";
		$sql.= " from tbl_junib {$where}";
		$ss = db_query_value($sql);
		$mm[bosu]   = $ss[bosu];
		$mm[gongga] = $ss[gongga];
		$mm[total_fee] = $ss[total_fee];
		$mm[ibgum] = $ss[ibgum];
		$mm[refund_fee] = $ss[refund];
		return $mm;
	}
...
<div id="content"">
  ...
  <div id="content-header">
    <div id="breadcrumb"> <a href="#" title="Go to Home" class="tip-bottom">양식관리 <a href="#" class="current">이전양식출력</a> </div>
  </div>
					...
					<tr>
					  <th style="text-align:left;margin-top:10px;">
						...
						<button type="button" class="btn btn-success" onclick="javascript:f_date25();"  style="background-color:#F29661;font-size:8pt;height:26px;">3개월</button>
						  ...
						  <option value="ijy_c_date" <?if($target_date=="ijy_c_date"){?>selected<?}?>>이전영수증출력일</option>
						  ...
						  <option value="ijy_c_date" <?if($target_date2=="ijy_c_date"){?>selected<?}?>>이전영수증출력일</option>
						  </select>&nbsp;
						  ...
						  &nbsp;&nbsp;&nbsp;&nbsp;비&nbsp;&nbsp;고&nbsp;&nbsp;&nbsp;&nbsp;<input type=text name="memo" value="<?=$memo?>" style="width:353px;">
						  &nbsp;&nbsp;&nbsp;&nbsp;보수액유무&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" id="bosu_ch" name="bosu_ch" <?if($bosu_ch=="Y"){?>checked<?}?> value="Y">
					  </th>
					  <th>
			  ...
			  <thead>
				<tr style="height:40px;vertical-align: middle;">
				  ...
				  <th style="text-align:right;border-left:0px;">
					...
					<button type="button" class="btn btn-success" onclick="javascript:f_2();"  style="background-color:#F29661;">권리증표지출력</button>&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="btn btn-success" onclick="javascript:f_3();"  style="background-color:#4374D9;">현금영수증발행</button>&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="btn btn-success" onclick="javascript:f_4();"  style="background-color:#F29661;">ERP연동</button>&nbsp;&nbsp;&nbsp;&nbsp;
					<button type="button" class="btn btn-success" onclick="javascript:f_5();f_6();"  style="background-color:blue;">팝빌바로가기</button>&nbsp;&nbsp;&nbsp;&nbsp;
	...
	<form name=ffm method="post">
        ...
        <div class="widget-box">
		  ...
		  <th><input type="checkbox" id="chx"></th>
                  ...
                  <th>취득자1</th>
                  <th>취득자2</th>
                  <th>보수액<br><?=f_money($ssm[bosu])?></th>

                  <th>공과금<br><?=f_money($ssm[gongga])?></th>
                  <th>총비용<br><?=f_money($ssm[total_fee])?></th>
                  <th>입금<br><?=f_money($ssm[ibgum])?></th>

                  <th style="background-color:#70AD47;color:white;">정산(환불금)<br><?=f_money($ssm[refund_fee])?></th>
                  <th style="background-color:#70AD47;color:white;">이전영수증출력일</th>
                  <th style="background-color:#70AD47;color:white;">현금영수증발행일</th>
                  <th>재무돌이비고</th>
                  <th>정산비고</th>
                </tr>
	...
	if($rows > 0){
		...
		<input type=hidden name="idx_<?=$i?>" value="<?=$row[idx]?>">
                ...
                <tr class="odd gradeX">
                  ...
                  <td style="text-align:center;"><input type="checkbox" name="ch[]" class="ch" value="<?=$row[idx]?>"></td>
                  ...
                  <td style="text-align:center;"><?=f_money(f_jung($row[aq1])+f_jung($row[ar1])+f_jung($row[as1])+f_jung($row[at1]))?></td><!--보수액//-->
                  <td style="text-align:center;"><?=f_money(f_jung($row[aj1])+f_jung($row[ak1])+f_jung($row[al1])+f_jung($row[am1])+f_jung($row[an1])+f_jung($row[ao1])+f_jung($row[ap1]))?></td><!--공과금//-->
...
?>
                  <td style="text-align:center;"><?=f_money($bosu+$gongga)?></td><!--총비용//-->

                  <td style="text-align:center;"><?=f_money($row[ai1])?></td><!--입금//-->

                  <td style="text-align:center;"><?=f_money($row[refund_fee])?><!--정산금//--></td>
                  <td style="text-align:center;"><?=f_date($row[ijy_c_date])?></td>
                  <td style="text-align:center;"><a href="javascript:f_delete(<?=$row[idx]?>);" style="color:red;"><?=f_date($row[hy_b_date])?></a></td>
                  <td style="text-align:center;"><textarea style="width:100px;" name=ijp_s_memo  cols=50 rows=1><?=$row[ijp_s_memo]?></textarea></td>
                  <td style="text-align:center;"><textarea style="width:100px;" name=ae1  cols=50 rows=1><?=$row[ae1]?></textarea></td>
                </tr>
	...
	<?$T++;$i++;}
              ...
              <tr class="title">
                <td colspan=16 align=center>내용이 없습니다.</td>
              </tr>
<?}?>
...
function f_excel(){
	if(<?=$rows_total?>==0){
		...
		frm.action = "ijun_excel.php";
		frm.method = "post";
		frm.submit();
		frm.action = "ijun.html";
	}
}
...
function f_delete(idx){
	if(confirm("선택된 현금영수증발행일을 삭제하시겠습니까?")){
			var url    ="./delete_ok.htm?idx="+idx;
...
function f_1(){  //영수증출력
		...
		if(count<=0){
			...
			var url    ="/report_form/ijun/youngsu.html";
			var title  = "listpop2";
...
function f_2(){  //권리등기증출력
		...
		if(count<=0){
			...
			var frm    = document.ffm;
			var url    ="/report_form/ijun/dunggi.html";
			var title  = "listpop21";
			var status = "toolbar=no,directories=no,scrollbars=yes,resizable=no,status=no,menubar=no,width=850, height=800, top=100,left=600";
...
function f_3(){  //현금영수증발행
		...
		if(count<=0){
			alert("체크박스을 선택 해 주세요.");
				...
				var frm    = document.ffm;
				var url    ="./hyungum_date_select.htm";
				var title  = "listpop21";
...
//		}else{
//			var frm    = document.ffm;
//			var url    ="./hyungum_ok.htm";
//			var title  = "listpop21";
//			var status = "toolbar=no,directories=no,scrollbars=yes,resizable=no,status=no,menubar=no,width=850, height=800, top=200,left=200";
//			var aa2 = window.open("", title,status);
//			frm.target = title;
//			frm.action = url;
//			frm.method = "post";
...
function f_4(){  //ERP연동
		...
		if(count<=0){
			...
			frm.action = "erp_excel.php";
			frm.submit();
		}
}

function f_5(){  //팝빌바로가기
		var url    ="./popbill_id.php";
		var title  = "listpop223";
		var status = "toolbar=no,directories=no,scrollbars=no,resizable=no,status=no,menubar=no,width=200, height=200, top=300,left=600";
		var aa4 = window.open(url,title,status);
		aa4.focus();
}

function f_6(){
		var url    ="https://www.popbill.com/Cash/Write/Cancel/Step1";
		var aa3 = window.open(url);
		aa3.focus();
}
