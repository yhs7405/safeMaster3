...
//	ini_set("display_errors", 1);
	...
	while($row = $stmt->fetch()){
		...
		//이전
		$sql = "select ifnull(count(h_idx),0) as ss,sum(ap1) ap1_s,sum(aq1) aq1_s,sum(ar1) ar1_s, sum(as1) as1_s,sum(ao1) ao1_s from tbl_junib where h_idx={$h_idx} and e1='{$row[e1]}' and d1='{$row[d1]}' group by h_idx ";
		//echo $sql;
		...
		if($ij[ss]==""){ 
			...
			$ij[ss]=$ij[ss];
			$ij_bosu   = $ij[ap1_s]+$ij[aq1_s];  //소유권이전보수료
			$ij_malso  = $ij[ar1_s]+$ij[as1_s];  //신탁말소보수료
			$ij_jejung = $ij[ao1_s];             //제증명
		}
		//설정
		$sql = "select ifnull(count(h_idx),0) as ss from tbl_suljung where h_idx={$h_idx}  and jijum_code='{$row[e1]}' and bank_code='{$row[d1]}' group by h_idx ";
		//echo $sql;
		$sj_cc = db_query_fetch($sql);
		if($sj_cc[ss]=="") $sj_cc[ss]=0;
	...
	while($row = $stmt->fetch()){
		...
		while($row1 = $stmt1->fetch()){
			...
			//}
			$sql = "select ifnull(ibgum_money1,0) as a1,ifnull(ibgum_money2,0) as a2 from tbl_sugum where h_idx={$h_idx}  and a1='{$row1[a1]}'  ";
			//echo $sql."<br>";
