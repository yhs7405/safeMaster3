<?	include ("../../include/dbconn.php");
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth(15);
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("E".$i, "서류수령일")
                 ->setCellValue("F".$i, "완증요청일")
                 ->setCellValue("G".$i, "완증수령일")
                 ->setCellValue("H".$i, "잔금일")
                 ->setCellValue("I".$i, "분양가")
                 ->setCellValue("J".$i, "발코니")
                 ->setCellValue("K".$i, "옵션1")
                 ->setCellValue("L".$i, "옵션2")
                 ->setCellValue("M".$i, "옵션3")
                 ->setCellValue("N".$i, "옵션4")
                 ->setCellValue("O".$i, "할인액")
                 ->setCellValue("P".$i, "부가세")
                 ->setCellValue("Q".$i, "프리미엄")
                 ->setCellValue("R".$i, "취득세과표")
                 ;
	$i++;
	...
	while($row = $stmt->fetch()){
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ...
                 ->setCellValue("E".$i, $row[doc_receive_date])
                 ->setCellValue("F".$i, $row[comp_req_date])
                 ->setCellValue("G".$i, $row[comp_rec_date])
                 ->setCellValue("H".$i, $row[balance_date])
								 ->setCellValue("I".$i, $row[bunyang_cost])
								 ->setCellValue("J".$i, $row[balkoni_cost])
								 ->setCellValue("K".$i, $row[option1_cost])
								 ->setCellValue("L".$i, $row[option2_cost])
								 ->setCellValue("M".$i, $row[option3_cost])
								 ->setCellValue("N".$i, $row[option4_cost])
								 ->setCellValue("O".$i, $row[discount_cost])
								 ->setCellValue("P".$i, $row[vat])
								 ->setCellValue("Q".$i, $row[pre_cost])
								 ->setCellValue("R".$i, $row[af1])
                 ;
	$i++;
	}
	 ...
	 $objPHPExcel->setActiveSheetIndex(0); 
	 $filename = iconv("UTF-8", "EUC-KR", "완납증명서입력_엑셀");
