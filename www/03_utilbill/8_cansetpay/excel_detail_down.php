  ...
  // 정산비고
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('K')->setWidth(50);
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ->setCellValue("A".$i, "동")
                 ->setCellValue("B".$i, "호")
                 ->setCellValue("C".$i, "취득자1주소")
                 ->setCellValue("D".$i, "취득자1주민번호")
                 ->setCellValue("E".$i, "취득자1")
                 ->setCellValue("F".$i, "취득자2주소")
                 ->setCellValue("G".$i, "취득자2주민번호")
                 ->setCellValue("H".$i, "취득자2")
                 ->setCellValue("I".$i, "등록세")
                 ->setCellValue("J".$i, "물건지지번")
                 ->setCellValue("K".$i, "물건지도로명")
                 ;
	$i++;
	...
	//현장정보 반환
		$sql2 = "select * from tbl_hyunjang_danji_info where h_idx='{$row[h_idx]}' and danji_name='{$row[b1]}' ";
		$kk = db_query_value($sql2);
		
		if($row[ao1]==""&&$row[ao1_toji]==""){
			$s_result = "";
		} else if ($row[ao1_toji]==""){
			$s_result = $row[ao1];
		} else {
			$s_result = $row[ao1]."(".$row[ao1_toji].")";
		}

     $objPHPExcel->setActiveSheetIndex(0)
                 ->setCellValue("A".$i, $row[h1])
                 ->setCellValue("B".$i, $row[i1])
                 ->setCellValue("C".$i, $row[l1])
                 ->setCellValue("D".$i, $row[k1])
                 ->setCellValue("E".$i, $row[j1])
                 ->setCellValue("F".$i, $row[o1])
                 ->setCellValue("G".$i, $row[n1])
                 ->setCellValue("H".$i, $row[m1])
								 ->setCellValue("I".$i, $s_result)
								 ->setCellValue("J".$i, $kk[jibun_addr])
								 ->setCellValue("K".$i, $kk[doro_addr])
                 ;
	$i++;
	}
	 ...
	 $objPHPExcel->setActiveSheetIndex(0); 
	 $filename = iconv("UTF-8", "EUC-KR", "말소-변경등록세신고납부_등록세신고용엑셀");
