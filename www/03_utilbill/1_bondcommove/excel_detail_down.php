  ...
  // ae1(정산비고)
	...
	$objPHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(20);
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ->setCellValue("A".$i, "이전채권요청일")
                 ->setCellValue("B".$i, "이전채권매입일")
	...
	while($row = $stmt->fetch()){
     ...
     $objPHPExcel->setActiveSheetIndex(0)
                 ->setCellValue("A".$i, $row[ijeon_purrec_date])
                 ->setCellValue("B".$i, $row[aj1_rec1_date])
                 ->setCellValue("C".$i, $row[a1])
                 ->setCellValue("D".$i, "이전")
                 ->setCellValue("E".$i, $row[j1])
                 ->setCellValue("F".$i, $row[k1])
                 ->setCellValue("G".$i, $row[me_prepur_cost1])
                 ;
		$i++;
		if($row[m1]!=""){
	     $objPHPExcel->setActiveSheetIndex(0)
	                 ->setCellValue("A".$i, $row[ijeon_purrec_date])
	                 ->setCellValue("B".$i, $row[aj1_rec2_date])
	                 ->setCellValue("C".$i, $row[a1])
	                 ->setCellValue("D".$i, "이전")
	                 ->setCellValue("E".$i, $row[m1])
	                 ->setCellValue("F".$i, $row[n1])
	                 ->setCellValue("G".$i, $row[me_prepur_cost2])
	                 ;
			$i++;
		}
	}
	 ...
	 $objPHPExcel->setActiveSheetIndex(0); 
	 $filename = iconv("UTF-8", "EUC-KR", "채권산정(이전채권)_이전채권매입용엑셀");
